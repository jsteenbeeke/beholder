...
public class FogOfWarPreviewRenderer implements FogOfWarShapeVisitor<String> {
	...
	public String visit(FogOfWarCircle circle) {
		return String.format("previewCircle('%s', 'rgba(0,0,255,0.1)', {'x': %d, 'y': %d, 'radius': %d, 'theta_offset': 0, 'theta_extent': (Math.PI*2)});\n",
	...
	public String visit(FogOfWarRect rect) {
		return String.format("previewRectangle('%s', 'rgba(0,0,255,0.1)', { 'x': %d, 'y': %d, 'width': %d, 'height': %d });\n",
				canvasId,
	...
	public String visit(FogOfWarTriangle triangle) {
		...
		return String.format("previewPolygon('%s', 'rgba(0,0,255,0.1)', { points: [%s] });\n", canvasId,
