...
public class UploadMapStep2Page extends AuthenticatedPage {
	...
	private final class UpdatePreviewBehavior extends AjaxFormComponentUpdatingBehavior {
	...
	public UploadMapStep2Page(final byte[] image, final String originalName) {
		...
		final TextField<String> nameField = new TextField<>("name", Model.of(originalName));
		nameField.setRequired(true);

		squareSizeField = new NumberTextField<>(
				"square", Model.of(5));
		...
		Form<ScaledMap> configureForm = new Form<ScaledMap>("configureForm") {
			...
			protected void onSubmit() {
				...
				mapService.createMap(getUser(), nameField.getModelObject(), squareSizeField.getModelObject(), image);
